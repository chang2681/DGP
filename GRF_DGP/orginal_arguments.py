...
def get_common_args():
    ...
    parser.add_argument('--lf_q', type=int,
                        default=3)  # 0 leader-follower ; 1 每个智能体一个； 2 只有局部leader-follower q函数 后面必须设置为False 3. 局部共享，每个angent只有一个局部q函数4
    parser.add_argument('--local_q_L1loss', action='store_false', default=False)  # !!!!!!!!!!!!!!!!!!!!!
    parser.add_argument('--label', action='store_true', default='CDS')
    ...
    # parser.add_argument('--beta2', type=float, default=1)#2.
    # parser.add_argument('--beta', type=float, default=.05)
    # parser.add_argument('--beta1', type=float, default=.98)#.5
    # parser.add_argument('--beta2', type=float, default=1)#2.
    ...
    # qmix
    parser.add_argument('--Qmix', action='store_false', default=False)
